...
int main()
{
   ...
   // cin>>tt;
    for(int tc=0;tc<tt;tc++)
    {
        ...
        vector<ll int>dp(n+1,1e18);
        vector<ll int>dp2(n+1,1e18);
        dp[1]=0;
        dp2[1]=0;

        //priority_queue<pair<ll int,ll int>,vector<pair<ll int,ll int>>,greater<pair<ll int,ll int>>>pq;
        set<pair<ll int,pair<ll int,ll int>>>pq;
        pq.insert({0,{1,0}});//0 denotes no discount coupn used till now and 1 deontes discount coupn used till now.
        while(!pq.empty())
        {
            auto node=pq.begin();
            pq.erase(pq.begin());

            ll int wt=node->first;
            int m=node->second.first;
            int type=node->second.second;
            for(auto x:graph[m])
            {
                int new_node=x[0];
                ll int new_wt=wt+x[1];

                if(type==0)
                {
                    if(dp[new_node]>new_wt)
                    {
                       pq.erase({dp[new_node],{new_node,  type}});
                        dp[new_node]=new_wt;
                        pq.insert({dp[new_node],{new_node,  type}});
                    }
                    //we can use discount coupn
                    ll int new_wt=wt;
                    new_wt+=(x[1]/2);
                    if(dp2[new_node]>new_wt)
                    {
                        pq.erase({dp2[new_node],{new_node,1}});
                        dp2[new_node]=new_wt;
                        pq.insert({dp2[new_node],{new_node,1}});
                    }
                }
                else
                {
                    if(dp2[new_node]>new_wt)
                    {
                        pq.erase({dp2[new_node],{new_node, type}});
                        dp2[new_node]=new_wt;
                        pq.insert({dp2[new_node],{new_node, type}});
                    }
                }
                /*if(dp[new_node]>new_wt)
                {
                    pq.erase({dp[new_node],new_node});
                    dp[new_node]=new_wt;
                    pq.insert({dp[new_node],new_node});
                }*/
            }
        }
       
        cout<<min(dp[n],dp2[n])<<endl;
