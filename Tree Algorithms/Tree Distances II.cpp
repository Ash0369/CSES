...
void dfs(int index,int parent,vector<vector<int>>&tree)
{
	...
	for(auto x:tree[index])
	{
		...
		dfs(x,index,tree);
		subtree[index]+=subtree[x];
		in[index]+=in[x]+subtree[x];
	}
}

void calculate(int index,int parent,vector<vector<int>>&tree,int n)
{
	if(parent!=-1)
	{
		dp[index]=dp[parent]-subtree[index]+n-subtree[index];
	}
	for(auto x:tree[index])
	{
		if(x==parent)
...
int main()
{
   ...
   // cin>>tt;
    for(int tc=0;tc<tt;tc++)
    {
		...
		// for(int i=1;i<=5;i++)
		// {
		// 	cout<<in[i]<<" "<<subtree[i]<<endl;
		// }

		dp[1]=in[1];
		calculate(1,-1,tree,n);
